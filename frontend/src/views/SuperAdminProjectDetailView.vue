...
import type { ChallengeProjectDetail, GovernanceProjectDetail, ProjectForm, RuleVersion } from '../types/challengeProject';
import {
...
<template>
  <WorkspaceShell role-label="超级管理员" workspace-title="平台管理工作台" :page-title="isNew ? '新建挑战项目' : '挑战项目详情'" description="编辑项目资料和成绩规则，并管理项目公开状态。" home-path="/super-admin" :navigation="navigation" :show-identity="false">
    <section class="project-admin-panel project-editor">
      <div class="project-detail-toolbar"><RouterLink class="project-back-link" to="/super-admin/projects">返回项目管理</RouterLink><span v-if="project" class="project-status" :data-status="project.status">{{ labelForProjectStatus(project.status) }}</span></div>
      ...
      <template v-else>
        ...
        <form class="project-form" @submit.prevent="save">
          <fieldset><legend>基本信息</legend><div class="project-form-grid"><label>项目名称<input v-model="form.name" required maxlength="200"></label><label>分类<input v-model="form.category" required maxlength="64" placeholder="输入项目分类"></label><label class="project-form-wide">项目说明<textarea v-model="form.description" rows="3"></textarea></label></div></fieldset>
          ...
          <fieldset><legend>成绩规则</legend><div class="project-form-grid"><label>成绩存储类型<select v-model="form.scoreStorageType"><option v-for="value in scoreStorageOptions" :key="value" :value="value">{{ labelForScoreStorageType(value) }}</option></select></label><label>成绩指标<select v-model="form.scoreIndicatorType"><option v-for="value in indicatorOptions" :key="value" :value="value">{{ labelForScoreIndicatorType(value) }}</option></select></label><label>比较方向<select v-model="form.comparisonDirection"><option v-for="value in directionOptions" :key="value" :value="value">{{ labelForComparisonDirection(value) }}</option></select></label><label>有效成绩规则<select v-model="form.effectiveScoreRule"><option v-for="value in effectiveOptions" :key="value" :value="value">{{ labelForEffectiveScoreRule(value) }}</option></select></label><label>成绩单位<input v-model="form.scoreUnit"></label><label>小数位<input v-model.number="form.decimalPlaces" type="number" min="0" max="6"></label><label class="project-form-wide">年级顺序<textarea v-model="form.gradeOrder" rows="2"></textarea></label><label class="project-checkbox"><input v-model="form.allowTie" type="checkbox">允许并列</label></div></fieldset>
