...
async function applyQueryAndLoad() { const q: Record<string,string> = {}; if (f.executionStatus) q.executionStatus = f.executionStatus; if (f.publicStatus) q.publicStatus = f.publicStatus; if (f.keyword?.trim()) q.keyword = f.keyword.trim(); if (page.value > 1) q.page = String(page.value); await router.replace({ query: q }); const flt = { executionStatus: f.executionStatus || undefined, publicStatus: f.publicStatus || undefined, keyword: f.keyword?.trim() || undefined }; loading.value = true; error.value = null; try { const r = await fetchActivities(flt, page.value - 1, 20); items.value = r.items; total.value = r.totalElements; } catch (e) { error.value = e instanceof ApiError ? e.message : '加载失败'; } finally { loading.value = false; } }
async function search() { page.value = 1; await applyQueryAndLoad(); }
async function handlePage(p: number) { page.value = p; await applyQueryAndLoad(); }
function fmt(iso: string | null) { return iso ? new Date(iso).toLocaleDateString('zh-CN') : ''; }
onMounted(() => { f.executionStatus = (route.query.executionStatus as string) || ''; f.publicStatus = (route.query.publicStatus as string) || ''; f.keyword = (route.query.keyword as string) || ''; const qp = Number(route.query.page); page.value = Number.isInteger(qp) && qp >= 1 ? qp : 1; void applyQueryAndLoad(); });
</script>
