<template>
  <div class="admindb"><h2>超级管理员后台</h2><p class="sub">{{ auth.user?.username }}</p>
    <el-row :gutter="16" class="stats"><el-col :xs="12" :sm="6" v-for="s in statCards" :key="s.key"><el-card shadow="hover" class="sc"><div class="num">{{ s.val }}</div><div class="lbl">{{ s.label }}</div></el-card></el-col></el-row>
    <div class="sec"><div class="sh"><h3>待审核申请</h3><el-button text type="primary" @click="$router.push('/admin/applications')">进入审核中心</el-button></div>
      <div v-if="recent.length===0" class="empty">暂无待审核申请</div>
...
const statCards = ref([{key:'total',label:'申请总数',val:0},{key:'submitted',label:'待审核',val:0},{key:'approved',label:'已批准',val:0},{key:'rejected',label:'已驳回',val:0}]);

onMounted(async () => {
  try {
    const [r, s] = await Promise.all([fetchAdminApplications({ status: 'SUBMITTED', size: 5 }), fetchAdminStats()]);
    recent.value = r.items;
    statCards.value[0].val = s.total; statCards.value[1].val = s.submitted; statCards.value[2].val = s.approved; statCards.value[3].val = s.rejected;
  } catch { /* silent */ }
});
...
.meta {color:#909399;font-size:13px;margin:0} .empty {color:#c0c4cc;padding:24px 0;text-align:center}
