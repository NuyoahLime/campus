<template>
  <div class="admindb"><h2>超级管理员后台</h2><p class="sub">{{ auth.user?.username }}</p>
    ...
    <div v-if="loadErr" class="err"><el-alert :title="loadErr" type="error" show-icon /><el-button size="small" @click="loadStats">重试</el-button></div>
...
async function loadStats() {
  ...
  } catch { loadErr.value = '加载统计数据失败，请重试'; }
