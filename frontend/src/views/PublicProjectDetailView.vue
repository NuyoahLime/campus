  ...
  <main class="project-public-page">
    ...
    <section class="project-public-content project-detail-page">
      ...
      <template v-else-if="project">
        <div class="project-detail-heading"><p class="eyebrow">{{ project.category }}</p><h1>{{ project.name }}</h1><p>{{ project.description || '暂无项目说明。' }}</p></div>
        <div class="project-detail-grid">
          ...
          <section class="project-detail-section"><h2>成绩规则</h2><dl><div><dt>成绩类型</dt><dd>{{ project.scoreStorageType }}</dd></div><div><dt>成绩指标</dt><dd>{{ project.scoreIndicatorType }}</dd></div><div><dt>成绩单位</dt><dd>{{ project.scoreUnit || '未填写' }}</dd></div><div><dt>比较方向</dt><dd>{{ project.comparisonDirection }}</dd></div><div><dt>有效规则</dt><dd>{{ project.effectiveScoreRule }}</dd></div><div><dt>允许并列</dt><dd>{{ project.allowTie ? '允许' : '不允许' }}</dd></div></dl></section>
