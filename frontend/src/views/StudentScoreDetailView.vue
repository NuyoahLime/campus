...
<template>
  <WorkspaceShell
    ...
    page-title="成绩详情"
    description="查看当前学生成绩及其所属活动的历史规则版本。"
    home-path="/student"
  ...
  >
    ...
    <div v-else-if="error" class="project-state project-state-error">
      <strong>{{ error }}</strong>
      <RouterLink class="secondary-button" to="/student/scores">返回成绩列表</RouterLink>
    </div>
    <template v-else-if="detail">
      <header class="student-score-detail-heading">
        <div>
          <p class="eyebrow">SCORE DETAIL</p>
        ...
        <RouterLink
          ...
          :to="`/student/appeals/new?scoreAttemptId=${detail.scoreAttemptId}`"
        >
          发起申诉
        </RouterLink>
      </header>
      <section class="student-score-detail-grid">
        <div class="student-score-detail-section">
          ...
          <dl>
            ...
            <div><dt>尝试次数</dt><dd>第 {{ detail.attemptNumber }} 次</dd></div>
            <div><dt>成绩时间</dt><dd>{{ detail.scoreBusinessTime ? new Date(detail.scoreBusinessTime).toLocaleString() : '未记录' }}</dd></div>
            <div><dt>状态</dt><dd><span class="student-score-status">{{ labelForStudentScoreStatus(detail.status) }}</span></dd></div>
          </dl>
        </div>
        <div class="student-score-detail-section">
          <h3>历史规则版本</h3>
          <dl>
            <div><dt>规则版本</dt><dd>V{{ detail.ruleVersionNumber }}</dd></div>
            <div><dt>规则版本 ID</dt><dd class="student-score-breakable">{{ detail.ruleVersionId }}</dd></div>
          </dl>
          <p class="student-score-rules">{{ detail.rulesText || '暂无规则说明。' }}</p>
        </div>
      </section>
    </template>
  </WorkspaceShell>
