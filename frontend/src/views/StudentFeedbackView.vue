...
import { listStudentFeedback } from '../api/studentFeedback';
...
async function load() {
  ...
  error.value = '';
  try {
    result.value = await listStudentFeedback(page.value, 20);
...
<template>
  <WorkspaceShell role-label="学生" workspace-title="学生个人工作台" page-title="意见反馈" description="提交并追踪自己的意见反馈。" home-path="/student" :navigation="navigation" :show-identity="false">
    <section class="student-score-panel">
      <header class="student-score-toolbar">
        <div>
          <p class="eyebrow">MY FEEDBACK</p>
          <h2>反馈记录</h2>
          <span>共 {{ result?.totalElements ?? 0 }} 条反馈</span>
        </div>
        <RouterLink class="primary-button" to="/student/feedback/new">提交反馈</RouterLink>
      </header>
      ...
      <div v-else-if="!items.length" class="project-state"><strong>暂无反馈记录</strong><p>有问题或建议时，可以在这里提交反馈。</p><RouterLink class="primary-button" to="/student/feedback/new">提交反馈</RouterLink></div>
        ...
        <table class="student-score-table">
          ...
          <tbody>
            <tr v-for="item in items" :key="item.feedbackId">
              ...
              <td><RouterLink class="registration-detail-link" :to="`/student/feedback/${item.feedbackId}`">查看详情</RouterLink></td>
            </tr>
          </tbody>
        </table>
      </div>
