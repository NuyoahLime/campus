...
class MediaControllerTest {
    ...
    class Register {
        ...
        void returns201() throws Exception {
            ...
            mvc.perform(post("/api/v1/media")
                            .contentType(MediaType.APPLICATION_JSON)
                            .content(mapper.writeValueAsString(
                                    new RegisterMediaRequest(UUID.randomUUID(), UUID.randomUUID(),
                                            "key", "f.jpg", "IMAGE", "JPG", 100, null, null))))
    ...
    class InternalReview {
        ...
        void returns200() throws Exception {
            UUID id = UUID.randomUUID();
            when(service.submitForInternalReview(id)).thenReturn(
                    new MediaResult(id, "PENDING_INTERNAL_REVIEW", "NOT_SUBMITTED"));
            mvc.perform(post("/api/v1/media/" + id + "/internal-review"))
                    .andExpect(status().isOk())
                    .andExpect(jsonPath("$.internalStatus").value("PENDING_INTERNAL_REVIEW"));
        }
    }

    @Nested
    class InternalApprove {
        @Test
        void returns200() throws Exception {
            UUID id = UUID.randomUUID();
            when(service.approveInternal(id)).thenReturn(
                    new MediaResult(id, "INTERNAL_APPROVED", "NOT_SUBMITTED"));
            mvc.perform(post("/api/v1/media/" + id + "/internal-approve"))
                    .andExpect(status().isOk())
                    .andExpect(jsonPath("$.internalStatus").value("INTERNAL_APPROVED"));
        }
    }
    ...
    class Errors {
        ...
        void notFound() throws Exception {
            when(service.submitForInternalReview(any()))
