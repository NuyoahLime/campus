...
class FeedbackApplicationServiceTest {
    ...
    UUID actorUserId;
    @BeforeEach void setUp() { actorUserId=UUID.randomUUID(); lenient().when(currentActor.requireUserId()).thenReturn(actorUserId); svc = new FeedbackApplicationService(repo, currentActor); }
